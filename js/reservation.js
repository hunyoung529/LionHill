...
document.getElementById("checkMove").addEventListener("click", function () {
  ...
  var totalPriceElement = document.querySelector(".grand_total span");
  var totalPrice = totalPriceElement.textContent || totalPriceElement.innerText;

  if (totalPrice.trim() === "0원") {
    alert("방을 선택해주세요."); // 조건이 맞으면 경고 메시지 표시
  } else {
    // "user" 키가 세션에 존재하는 경우
    if (userSession !== null) {
      //로그인중임
